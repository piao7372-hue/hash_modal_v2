...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate formal Stage 3 semantic cache artifacts for one dataset."
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--write-summary",
        action="store_true",
        help="Write validator_summary.json into the formal semantic cache directory.",
    )
...
def main() -> int:
    ...
    summary = build_validator_summary(config, args.dataset)
    if args.write_summary:
        write_semantic_validator_summary(config, args.dataset, summary)
